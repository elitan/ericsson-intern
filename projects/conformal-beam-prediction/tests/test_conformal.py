...
class TestAlphaSweep:
    def test_coverage_decreases_with_alpha(self):
        model = _make_model()
        cal_loader = _make_loader(n=300)
        test_loader = _make_loader(n=200)
        test_labels = []
        for _, lbl in test_loader:
            test_labels.extend(lbl.numpy().tolist())

        alphas = [0.05, 0.1, 0.2, 0.3]
        coverages = []
        mean_sizes = []
        for alpha in alphas:
            th, _ = calibrate(model, cal_loader, alpha=alpha)
            ps = predict_sets(model, test_loader, th)
            coverages.append(coverage(ps, test_labels))
            mean_sizes.append(set_sizes(ps).mean())

        for i in range(len(alphas) - 1):
            assert coverages[i] >= coverages[i + 1] - 0.05
            assert mean_sizes[i] >= mean_sizes[i + 1] - 0.5
