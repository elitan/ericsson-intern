#!/usr/bin/env python3
"""Cross-domain experiment: Train on day (31+32), test on night (33+34)."""
...
def create_cross_domain_loaders(
    ...
    """Create dataloaders for cross-domain experiment."""
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Test Loss (night): {test_metrics['loss']:.4f}")
        print(f"Top-1: {test_metrics['top_1_accuracy']:.2%}")
        print(f"Top-3: {test_metrics['top_3_accuracy']:.2%}")
        print(f"Top-5: {test_metrics['top_5_accuracy']:.2%}")
        ...
        if test_metrics["top_1_accuracy"] > best_acc:
            ...
            torch.save({
                ...
                "train_scenarios": args.train_scenarios,
                "test_scenarios": args.test_scenarios,
            }, save_dir / "cross_domain_model.pt")
            print(f"Saved best model (top-1: {best_acc:.2%})")

    print(f"\n=== Cross-Domain Results ===")
    print(f"Train: {args.train_scenarios} (day)")
    print(f"Test: {args.test_scenarios} (night)")
