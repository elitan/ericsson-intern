...
\textbf{Computational overhead.} CP adds minimal computational cost: the calibration step computes a single quantile (one-time), and prediction requires comparing $K$ softmax values against a threshold (per-prediction). Both operations are negligible compared to neural network inference.
