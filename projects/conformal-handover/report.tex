...
\begin{enumerate}
    ...
    \item We compare static CP, ACI~\cite{gibbs2021}, weighted CP, and confidence-gated triggered ACI under the same base predictor and KPI mapping.
...
& ACI & $\mathbf{.888}$ & 5.98 & $\mathbf{.073}$ & $.600$ \\
& Triggered ACI & $.843$ & 5.40 & $.121$ & $.542$ \\
& Weighted CP & $.691$ & 2.60 & $.307$ & $.168$ \\
\midrule
\multirow{4}{*}{Regime}
& Static CP & $.780$ & 2.60 & $.218$ & $.167$ \\
& ACI & $\mathbf{.892}$ & 4.48 & $\mathbf{.084}$ & $.413$ \\
& Triggered ACI & $.868$ & 4.20 & $.109$ & $.384$ \\
& Weighted CP & $.787$ & 2.69 & $.210$ & $.174$ \\
\bottomrule
\end{tabular}
\end{table}

Paired seed deltas confirm hard-shift reliability gains: ACI vs static is $+19.6$pp coverage on shadow shift (95\% CI $[19.3,19.8]$) and $+11.2$pp on regime switch (95\% CI $[10.4,12.2]$). Triggered ACI keeps most of that gain ($+15.1$pp shadow, $+8.8$pp regime) while reducing overhead versus full ACI by $5.87$pp and $2.91$pp, respectively.
...
Top-3 & $.626$ & 3.00 \\
Static CP & $.762$ & 4.59 \\
ACI & $\mathbf{.880}$ & 15.13 \\
Triggered ACI & $.819$ & 9.48 \\
Weighted CP & $.738$ & 4.19 \\
\bottomrule
\end{tabular}
\end{table}
...
Trace-bootstrap on Irish confirms the reliability-cost pattern: ACI vs static gives $+11.8$pp coverage (95\% CI $[5.4,20.0]$) with $+10.53$ set size (95\% CI $[3.98,19.61]$), while triggered ACI gives $+5.8$pp coverage (95\% CI $[2.1,10.7]$) with $+4.89$ set size (95\% CI $[1.18,10.32]$). Triggered ACI versus full ACI loses $6.1$pp coverage (95\% CI $[-11.5,-2.0]$) but cuts set size by 5.65 (95\% CI $[-12.13,-0.99]$). A speed-bin breakdown shows high-speed traces are hardest (static $68.9\%$, weighted $67.0\%$) while ACI remains highest at $83.8\%$ and triggered ACI reaches $77.5\%$.
...
\midrule
$\le .20$ & Static CP ($.692$) & Weighted CP ($.787$) \\
$\le .40$ & Static CP ($.692$) & Triggered ACI ($.868$) \\
$\le .60$ & ACI ($.888$) & ACI ($.892$) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{System implications.} Reliability gains translate to lower RLF proxy but require explicit overhead budgeting. Table~\ref{tab:overhead-caps} gives a simple control policy: low-budget mode prefers static or weighted CP, medium-budget mode uses triggered ACI, and high-budget mode uses full ACI.
