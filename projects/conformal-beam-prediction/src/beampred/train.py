...
from beampred.config import LR, EARLY_STOP_PATIENCE, N_NARROW_BEAMS
...
def train_model(train_loader, val_loader, device="cpu", verbose=True, model=None):
    ...
    cosine_sched = CosineAnnealingLR(optimizer, T_max=config.EPOCHS - WARMUP_EPOCHS)
    ...
    for epoch in range(config.EPOCHS):
        ...
        if verbose and (epoch + 1) % 5 == 0:
            remaining = epoch_dur * (config.EPOCHS - epoch - 1)
            print(
