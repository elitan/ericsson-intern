...
def plot_coverage_vs_setsize_tradeoff(alpha_sweep):
    setup_style()
    fig, ax = plt.subplots(figsize=(8, 5))

    std_data = alpha_sweep["std"]
    ba_data = alpha_sweep["beam_aware"]

    std_sz = [d["mean_set_size"] for d in std_data]
    std_cov = [d["coverage"] for d in std_data]
    ba_sz = [d["mean_set_size"] for d in ba_data]
    ba_cov = [d["coverage"] for d in ba_data]

    ax.plot(std_sz, std_cov, "o-", color="C0", linewidth=2, markersize=8, label="Standard CP")
    ax.plot(ba_sz, ba_cov, "s--", color="C3", linewidth=2, markersize=8, label="Beam-aware CP")

    for d in std_data:
        ax.annotate(f"α={d['alpha']:.2f}", (d["mean_set_size"], d["coverage"]),
    ...
    ax.set_title("Coverage vs Set Size Trade-off (α sweep)")
    ax.legend()
    ax.axhline(0.9, color="gray", linestyle=":", alpha=0.5, label="90% target")
    save_fig(fig, "alpha-sweep-tradeoff.png")


def plot_conditional_coverage(conditional_coverage, target=0.9):
    setup_style()
    fig, ax = plt.subplots(figsize=(8, 5))

    labels = [cc["bin_label"] for cc in conditional_coverage]
    std_covs = [cc["std_coverage"] for cc in conditional_coverage]

    x = np.arange(len(labels))
    width = 0.5
    ax.bar(x, std_covs, width, label="Standard CP", color="C0", edgecolor="black", linewidth=0.5)
    ax.axhline(target, color="red", linestyle="--", linewidth=1.5, label=f"{target*100:.0f}% target")

    for i, sc in enumerate(std_covs):
        ax.text(i, sc + 0.01, f"{sc:.2f}", ha="center", va="bottom", fontsize=9)

    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_xlabel("Distance bin (quartile)")
    ax.set_ylabel("Coverage")
    ax.set_title("Standard CP: Conditional Coverage Gap")
    ax.legend(fontsize=8)
    ax.set_ylim(0, 1.15)
    save_fig(fig, "conditional-coverage.png")


def plot_group_conditional_comparison(group_conditional_coverage, target=0.9):
    setup_style()
    fig, ax = plt.subplots(figsize=(8, 5))
    labels = [cc["bin_label"] for cc in group_conditional_coverage]
    std_covs = [cc["std_coverage"] for cc in group_conditional_coverage]
    group_covs = [cc["group_coverage"] for cc in group_conditional_coverage]
    x = np.arange(len(labels))
    width = 0.35
    ax.bar(x - width / 2, std_covs, width, label="Standard CP", color="C0", edgecolor="black", linewidth=0.5)
    ax.bar(x + width / 2, group_covs, width, label="Group-conditional CP", color="C2", edgecolor="black", linewidth=0.5)
    ax.axhline(target, color="red", linestyle="--", linewidth=1.5, label=f"{target*100:.0f}% target")
    for i, (sc, gc) in enumerate(zip(std_covs, group_covs)):
        ax.text(i - width / 2, sc + 0.01, f"{sc:.2f}", ha="center", va="bottom", fontsize=7)
        ax.text(i + width / 2, gc + 0.01, f"{gc:.2f}", ha="center", va="bottom", fontsize=7)
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_xlabel("Distance bin (quartile)")
    ax.set_ylabel("Coverage")
    ax.set_title("Standard vs Group-Conditional CP Coverage")
    ax.legend(fontsize=8)
    ax.set_ylim(0, 1.15)
    save_fig(fig, "group-conditional-coverage.png")
...
def generate_all_figures(all_results, mlp_results=None, error_results=None,
                         ...
                         alpha_sweep=None, conditional_coverage=None,
                         group_conditional_coverage=None):
    ...
    if alpha_sweep is not None:
        plot_coverage_vs_setsize_tradeoff(alpha_sweep)
    if conditional_coverage is not None and len(conditional_coverage) > 0:
        plot_conditional_coverage(conditional_coverage)
    if group_conditional_coverage is not None and len(group_conditional_coverage) > 0:
        plot_group_conditional_comparison(group_conditional_coverage)
