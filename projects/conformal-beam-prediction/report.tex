...
\begin{abstract}
ML-based mmWave beam prediction lacks reliability guarantees. We evaluate conformal prediction (CP) on two DeepMIMO scenarios at 28~GHz, finding: (1)~CP benefit is scenario-dependent---set size 1.19 on boston5g\_28 (83.5\% accuracy) vs.\ 1.00 on O1\_28 (92.2\%); (2)~standard CP exhibits conditional coverage gaps (79\% at short distances vs.\ 90\% target); (3)~group-conditional CP closes this gap with modest overhead (set size 1.29). Results show CP value depends on base model accuracy and channel complexity.
\end{abstract}
